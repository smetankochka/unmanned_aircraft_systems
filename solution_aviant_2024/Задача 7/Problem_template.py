...
def detectCar(image, camera_zxy):
    ...
    color_to_find = np.uint8([[[42, 207, 237]]])
...
def camera_processing_thread():
    global points_array, pt_index, is_dropped, array, flag
    # Параметры камеры
    F = 6.1 * 1e-3  # Фокусное расстояние
    ...
    while running:
        ...
        if ret:
            ...
            if X > 423-89+23 and abs(abs (Z)-748) < 10 and is_dropped == False:
                sendDropCargo ()
                is_dropped = True

            # Обработка изображения
                
            detected_coordinates = detectCar(frame, (Z, X, H-18.7))
            if detected_coordinates is not None:
                flag = True
                array.append(detected_coordinates)
                print(True)
            if flag and array:
                pos = calculateCarPosition(array)
                print(pos)
                cv2.putText(frame, f"{pos[0]}, {pos[1]}", (100, 100), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 255, 0), 2)
